...
Regular views need to be differentiated from materialized views as the use cases might be different. However, we don't want to create a hard distinction between both as it could cause confusion for users being introduced to the concept of views. For this purpose, an indicator can be added next to the view icon to show that it is of the type 'Materialized'.
