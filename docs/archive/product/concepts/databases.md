...
Within a database, you can store different kinds of data in different [Schemas](./schemas.md). Data is stored in [Tables](./tables.md) and can also be manipulated through [Views](./views.md).

## Usage
- Mathesar will create a new database for users unless one is already provided.
- We encourage users to use [Schemas](./schemas.md) for storing different sets of unrelated data rather than creating multiple databases.