...
The for to _edit_ existing DB connections, is much more straightforward and thus is not covered here.
...
- `db` is pre-filled as a suggested database name only if no user database connections
